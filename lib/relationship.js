...
_.extend(Finder.prototype, {
  ...
  handleRel: function (person, rel) {
    ...
    this.crawled += 1
    if (['father', 'mother', 'parent'].indexOf(rel.type) === -1) {
  ...
  finishIt: function (person, rel) {
    ...
    this.getDisplay(rel.id, function (display) {
      line.push({rel: has.rel, id: rel.id, display: display, link: rel.type})
      while (has.rel !== null) {
        has = self.ancestors[has.from]
        line.push({rel: has.rel, id: has.id, display: has.display})
      }
      line.unshift({rel: person.rel, id: person.id, display: person.display})
